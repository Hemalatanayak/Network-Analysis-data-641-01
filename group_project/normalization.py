import os
import json
from imdb_scraper import normalize_movie_role, normalize_movie_subrole, is_feature_film
                ...
                try:
                    with open(file_path, 'r', encoding='utf-8') as file:
                        ...
                        if not data.get('full_credits'):
                            ...
                            continue
                        title_id = data['title_uri'].split('/')[4]  # Extract title ID from title URI
                        if not is_feature_film(title_id):
                            if os.path.exists(os.path.join(normalized_data_dir, file_name)):
                                os.remove(os.path.join(normalized_data_dir, file_name))
                            print(f"Skipping non-feature film: {file_path}")
                            continue
...
#                             continue
#                         title_id = data['title_uri'].split('/')[4]  # Extract title ID from title URI
#                         if is_feature_film(full_credit['id']):
#                             for full_credit in data['full_credits']:
#                                 original_role = full_credit['role']
#                                 normalized_role = normalize_movie_role(original_role)
#                                 full_credit['normalized role'] = normalized_role
